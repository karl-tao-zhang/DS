...
import matplotlib.dates as md  # 处理画图里面的日期


def dmy2ymd(dmy):
    # unicode有几种编码,包括USC-4,UTF-8等默认读进来的是UTF-8类型,UTF-8的一个字符是1至4个字节
    # UCS-4每个字符是固定4字节,符合数组同质性的要求
    dmy = str(dmy, encoding='utf-8')  # 把utf-8转成UCS-4
    date = dt.datetime.strptime(dmy, '%d-%m-%Y').date()  # 告诉他怎么分析日期
...
ax.xaxis.set_major_locator(
    md.WeekdayLocator(byweekday=md.MO))  # 主刻度画在周一上午
ax.xaxis.set_minor_locator(md.DayLocator())
ax.xaxis.set_major_formatter(md.DateFormatter('%d %b %Y'))  # 主刻度标签日期格式化器
mp.tick_params(labelsize=10)
mp.grid(linestyle=':')

# 把numpy里的M8[D]类型转换成md.datetime.datetime类型:
