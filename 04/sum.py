...
wdays, opening_prices, highest_prices, lowest_prices, closing_prices = np.loadtxt(
    '../data/aapl.csv', delimiter=',', usecols=(1, 3, 4, 5, 6), unpack=True, converters={1: dmy2wday}
)
# 统计前三周(工作日)的信息
wdays = wdays[:19]
...
# print(np.where(wdays == 4))
indeces = np.arange(first_monday, last_friday + 1)
indices = np.split(indeces, 3)  # 等分成3段,3个每周下标
print(indices)


def week_summary(indice):  # 参数indice是一周的下标值
    opening_price = opening_prices[indice[0]]
    highest_price = np.max(np.take(highest_prices, indice))
    lowest_price = np.min(np.take(lowest_prices, indice))
...
print(summaries)
# 保存文件
np.savetxt('../data/summary_new.csv', summaries, delimiter=',', fmt='%g')
# fmt='%g'为紧凑浮点型
