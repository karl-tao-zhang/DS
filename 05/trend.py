...
dates, opening_prices, highest_prices, lowest_prices, closing_prices = np.loadtxt(
    '../data/aapl.csv', delimiter=',', usecols=(1, 3, 4, 5, 6), unpack=True, dtype=np.dtype('M8[D],f8,f8,f8,f8'), converters={1: dmy2ymd}
)
...
fall = closing_prices - opening_prices <= 0.01
...
mp.bar(dates, closing_prices - opening_prices,
       0, opening_prices, color=fc, edgecolor=ec)
...
# 画压力线和支撑线
mp.scatter(dates, resistance_points, c='orangered', alpha=0.5, s=80, zorder=2)
mp.scatter(dates, support_points, c='limegreen', alpha=0.5, s=80, zorder=2)
