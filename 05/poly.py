...
warnings.filterwarnings('ignore', category=np.RankWarning)  # 忽略一些警告
...
dates, bhp_closing_prices = np.loadtxt('../data/bhp.csv', delimiter=',', usecols=(
    1, 6), unpack=True, dtype=np.dtype('M8[D],f8'), converters={1: dmy2ymd})
dates, vale_closing_prices = np.loadtxt('../data/vale.csv', delimiter=',', usecols=(
    1, 6), unpack=True, dtype=np.dtype('M8[D],f8'), converters={1: dmy2ymd})
...
reals = roots[np.isreal(roots)].real  # np.isreal()返回的是一个布尔型数组,True对应的就是实根
#.real用来去掉实根的虚部0j
#[]布尔值做掩码

# 峰值数组,装的是第一个点,最后一个点,以及拐点

peeks = [[days[0], np.polyval(p, days[0])]]  # 第一个点
# peeks这是一个列表
for real in reals:
    # 在第一天和最后一天区间内的reals值才保留
    if days[0] <= real and real <= days[-1]:
        peeks.append([real, np.polyval(p, real)])

peeks.append([days[-1], np.polyval(p, days[-1])])  # 第一个点
peeks.sort()
peeks = np.array(peeks)  # 把peeks变成数组
# 将peeks拆成日期和价格两个一维数组
dates, prices = np.hsplit(peeks, 2)
dates = dates.astype(int).astype('M8[D]').astype(
    md.datetime.datetime)  # 方程的解释浮点型的需要转型

# 用箭头连接着几个拐点
for i in range(1, dates.size):
    mp.annotate('', xytext=(dates[i - 1], prices[i - 1]), xy=(dates[i], prices[i]),
                size=40, arrowprops=dict(arrowstyle='fancy', color='orangered', alpha=0.25))
